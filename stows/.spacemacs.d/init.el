...
(defconst mac? (eq system-type 'darwin)
  "Are we on a macOS machine?")
(defconst config-env (let ((env (getenv "CONFIG_ENV")))
                       (if env env "base"))
...
This function should only modify configuration layer settings."
  (setq-default
   ...
   '(yaml (haskell :variables haskell-completion-backend'dante
                   haskell-enable-hindent t)
          (python :variables python-backend'lsp python-lsp-server
                  'pyright python-formatter 'black python-format-on-save
                  t python-sort-imports-on-save t python-test-runner
                  'pytest)
          fasd
          emacs-lisp
          ...
          org
          (shell :variables shell-default-shell
                 (config-get config-env "shell-default-shell")
   ...
   '(
     ...
     (term-cursor :location (recipe :fetcher github
...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   dotspacemacs-show-trailing-whitespace
   t
   ...
   ;; (default t)
   dotspacemacs-use-clean-aindent-mode
   t
   ...
   dotspacemacs-home-shorten-agenda-source
   nil
...
lsp mode. For details see: https://github.com/flycheck/flycheck/issues/1762"
    (or (alist-get property
                   (alist-get checker my/flycheck-local-cache))
  ...
  (setq dotspacemacs-scratch-buffer-persistent
        t)
